...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [hyatt-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/travel/hyatt/cmd/hyatt-pp-cli
---

# World of Hyatt — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `hyatt-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install hyatt --cli-only
   ```
2. Verify: `hyatt-pp-cli --version`
...
```bash
hyatt-pp-cli which "<capability in your own words>"
```
...
Run `hyatt-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  hyatt-pp-cli calendars --spirit-code example-value --agent --select id,name,status
  ```
...
```
hyatt-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
hyatt-pp-cli feedback --stdin < notes.txt
hyatt-pp-cli feedback list --json --limit 10
```

Entries are stored locally at `~/.local/share/hyatt-pp-cli/feedback.jsonl`. They are never POSTed unless `HYATT_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `HYATT_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
hyatt-pp-cli profile save briefing --json
hyatt-pp-cli --profile briefing calendars --spirit-code example-value
hyatt-pp-cli profile list --json
hyatt-pp-cli profile show briefing
hyatt-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `hyatt-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add hyatt-pp-mcp -- hyatt-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which hyatt-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   hyatt-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `hyatt-pp-cli <command> --help`.
