# World of Hyatt Printed CLI Agent Guide

This directory is a generated `hyatt-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hyatt-pp-cli doctor --json
hyatt-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hyatt-pp-cli which "<capability>" --json
hyatt-pp-cli <command> --help
```
...
```bash
hyatt-pp-cli <command> --help
hyatt-pp-cli <command> --dry-run --agent
```
