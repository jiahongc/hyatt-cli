...
import (
	...
	mcptools "github.com/jiahongc/hyatt-cli/internal/mcp"
	"github.com/mark3labs/mcp-go/server"
)

// Transport selection order: --transport flag, then HYATT_MCP_TRANSPORT env,
...
func main() {
	...
	case "http":
		...
		fmt.Fprintf(os.Stderr, "hyatt-mcp serving MCP over streamable HTTP at %s\n", *addr)
...
// defaultTransport reads HYATT_MCP_TRANSPORT env when set, otherwise falls back
...
func defaultTransport() string {
	if t := os.Getenv("HYATT_MCP_TRANSPORT"); t != "" {
		return t
	}
	return "stdio"
}
