...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "hyatt-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HYATT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HYATT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hyatt-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/hyatt-cli/feedback.jsonl.
When ` + "`HYATT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HYATT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hyatt-cli feedback list
  hyatt-cli feedback list --limit 5
  hyatt-cli feedback list --json`,
