...
Resource scoping:
  ...
  hyatt-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  hyatt-cli sync --full

  # Incremental sync: only records from the last 7 days
  hyatt-cli sync --since 7d

  # Parallel sync with 8 workers
  hyatt-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  hyatt-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "hyatt-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
			...
			// Under HYATT_CLI_VERIFY=1 (mock/dry-run), all goroutines
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "calendars":
		...
		return db.UpsertCalendars(typedData)
	case "hotels":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertHotels(typedData)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"calendars": "/explore-hotels/rate-calendar",
		"hotels":    "/explore-hotels/service/hotels",
	}
