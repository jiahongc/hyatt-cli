...
Resource scoping:
  ...
  hyatt-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  hyatt-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  hyatt-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  hyatt-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  hyatt-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "hyatt-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
