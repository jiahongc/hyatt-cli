...
Run 'api <interface>' to see that interface's methods.`,
  ...
  hyatt-cli api <interface-name>`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				...
				for _, child := range root.Commands() {
					if child.Hidden && strings.ToLower(child.Name()) == target {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "\nUse 'hyatt-cli %s <method> --help' for details.\n", child.Name())
						return nil
					}
				}
				return fmt.Errorf("interface %q not found. Run 'hyatt-cli api' to list all interfaces", args[0])
			}
			...
			fmt.Fprintln(cmd.OutOrStdout(), "\nUse 'hyatt-cli api <interface>' to see methods.")
			return nil
		},
	}
