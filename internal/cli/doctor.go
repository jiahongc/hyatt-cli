...
// first leaf that (a) carries the `hyatt:endpoint` annotation, so it actually
...
func suggestReadCommand(root *cobra.Command) string {
	...
	walk = func(cmd *cobra.Command, path []string) {
		...
		for _, child := range cmd.Commands() {
			...
			// Recurse even into Hidden parents: CLIs mark raw
...
func isSuggestableReadLeaf(cmd *cobra.Command) bool {
	...
	// supposed to avoid.
	if cmd.Annotations["hyatt:endpoint"] == "" {
		return false
	}
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hyatt-cli doctor
  hyatt-cli doctor --json
  hyatt-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "hyatt-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".hyatt.com"
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/explore-hotels/rate-calendar?spiritCode=kulal&startDate=2026-09-01&endDate=2026-09-02&rooms=1&adults=1&kids=0&rate=Standard&vrcEnabled=true"
			...
			// inherits HYATT_CLI_VERIFY=1 (parent shell, CI runner, container
			...
			if cliutil.IsVerifyEnv() {
				if cliutil.IsVerifyLiveHTTPEnv() {
					...
					report["verify_mode"] = "INFO ACTIVE — mutating HTTP verbs short-circuit (HYATT_CLI_VERIFY=1; no network calls for DELETE/POST/PUT/PATCH)"
				}
			} else {
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run hyatt-cli auth login --chrome"
	}
	var proof doctorBrowserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run hyatt-cli auth login --chrome"
	}
	if proof.APIName != "hyatt" {
		return false, "proof belongs to a different CLI"
	}
	if proof.CookieDomain != ".hyatt.com" {
		return false, "proof belongs to a different cookie domain"
	}
	if proof.ValidationPath != "/explore-hotels/service/hotels" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'hyatt-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'hyatt-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'hyatt-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'hyatt-cli sync' to refresh."
	}
	return report
}
