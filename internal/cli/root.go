...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "hyatt-cli",
		Short: `Hyatt CLI — Hyatt award availability as a scriptable, local, agent-readable CLI.`,
		Long: `Hyatt CLI — Hyatt award availability as a scriptable, local, agent-readable CLI.
...
Health check: run 'hyatt-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("hyatt-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'hyatt-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAwardsCmd(flags))
	...
	rootCmd.AddCommand(newHotelsPromotedCmd(flags))
