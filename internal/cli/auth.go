...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Hyatt",
...
Use --chrome to read cookies from Chrome for .hyatt.com.
...
profile by name when the installed backend supports it.`,
		Example: `  hyatt-cli auth login --chrome
  hyatt-cli auth login --browser
  hyatt-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  hyatt-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  hyatt-cli auth login --browser\n")
				return nil
			}
...
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  hyatt-cli auth refresh",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionTargetURL(cfg *config.Config) string {
	baseURL := strings.TrimRight("https://www.hyatt.com", "/")
	if cfg != nil && cfg.BaseURL != "" {
		baseURL = strings.TrimRight(cfg.BaseURL, "/")
	}
	validationPath := strings.TrimSpace("/explore-hotels/service/hotels")
	if validationPath == "" {
		return baseURL
	}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  hyatt-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: .hyatt.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  hyatt-cli auth set-token eyJhbGciOi...",
...
func browserSessionProofPath(cfg *config.Config) string {
	if cfg == nil || cfg.Path == "" {
		...
		return filepath.Join(home, ".config", "hyatt-cli", "browser-session-proof.json")
	}
...
func browserSessionProofStatusForAuth(cfg *config.Config, authHeader string) (bool, string) {
	...
	if proof.APIName != "hyatt" || proof.CookieDomain != ".hyatt.com" || proof.ValidationPath != "/explore-hotels/service/hotels" {
