...
// short-circuit emitted by client.go.tmpl. Under HYATT_CLI_VERIFY=1
...
func TestClient_VerifyShortCircuit_MutatingVerbs(t *testing.T) {
	for _, verb := range []string{"DELETE", "POST", "PUT", "PATCH"} {
		...
		t.Run(verb, func(t *testing.T) {
			...
			if got, _ := env["__hyatt_verify_synthetic__"].(bool); !got {
				t.Fatalf("envelope must include __hyatt_verify_synthetic__: true; got %v", env)
			}
...
func TestClient_VerifyShortCircuit_LiveHTTPOptIn(t *testing.T) {
	t.Setenv("HYATT_CLI_VERIFY", "1")
	t.Setenv("HYATT_CLI_VERIFY_LIVE_HTTP", "1")
	c, rec := newClientWithRecorder(t)
...
// HYATT_CLI_VERIFY=1, because they cannot mutate remote state.
...
// under HYATT_CLI_VERIFY=1, because the operation does not mutate
