...
const BinaryResponseHeader = "X-Hyatt-CLI-Binary-Response"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "hyatt-cli", "http")
...
// real transport even under HYATT_CLI_VERIFY=1 without LIVE_HTTP=1.
...
// HYATT_CLI_VERIFY=1 (without LIVE_HTTP=1 opt-in), generated
...
// verify mode. The __hyatt_verify_synthetic__ sentinel is namespace-
// reserved (no real API uses __hyatt_*) so downstream consumers
...
func verifyShortCircuitEnvelope(method, path string) json.RawMessage {
	body, _ := json.Marshal(map[string]any{
		"__hyatt_verify_synthetic__": true,
		"status":                     "noop",
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// that sets HYATT_CLI_VERIFY=1) drives a mutating verb without
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export HYATT_COOKIES=<your-token> or hyatt-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.HyattCookies)
	}
